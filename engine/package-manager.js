...
const createPackage = async (req, res) => {
    ...
    try {
        result = await insertPackage(data, req.user.id);
    } catch (e) {
        console.log(e);
        return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({
            error: true,
            message: 'Error inserting the new package.',
        });
    }
    ...
    if (result) { // If the coupon has been created
        ...
        if(data.coupons) {
            for (let i = 0; i < data.coupons.length; i++) {
                for (let j = 0; j < data.coupons[i].quantity; j++){
                    try {
                        const newTokenCoupon = await CouponTokenManager
                            .getTokenByIdCoupon(data.coupons[i].id);
                        console.log('newTokenCoupon', newTokenCoupon)
                        await CouponTokenManager.updateCouponToken(newTokenCoupon.dataValues.token, data.coupons[i].id, null, result.dataValues.id, null)
                        if (data.categories.length > 0) {
                            for (let i = 0; i < data.categories.length; i++) {
                                await CategoriesPackageManager.assignCategory({
                        ...
                        return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({
                            error: true,
                            message: 'Error update the token.'
                        });
                    }
...
const insertPackage = (pack, broker) => {
    ...
    return new Promise((resolve, reject) => {
        Package.create({
            ...
            price: pack.price,
            visible_from: pack.visible_from === null ? null : Number(pack.visible_from),
            valid_from: Number(pack.valid_from),
            valid_until: pack.valid_until === null ? null : Number(pack.valid_until),
